...
//	zipInfo.dosDate = (unsigned long) current;
  ...
  if( err ) {
    LogError(@"Cannot get attributes of file '%@': %@", file, [err localizedDescription]);
    return NO;
  }
	if( attr )
	{
...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	if( !_unzFile ) {
    [self OutputErrorMessage:@"unzOpen"];
    return NO;
  }
  unz_global_info globalInfo = {0};
	if( unzGetGlobalInfo(_unzFile, &globalInfo )!=UNZ_OK ) {
    [self OutputErrorMessage:@"unzGetGlobalInfo"];
    return NO;
  }
  return YES;
}
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
	...
	if( ret!=UNZ_OK )
	{
		[self OutputErrorMessage:@"unzGoToFirstFile"];
    return NO;
	}
	...
	do{
		...
		if( ret!=UNZ_OK )
		{
			[self OutputErrorMessage:@"unzOpenCurrentFile"];
			success = NO;
			break;
		}
		...
		if( ret!=UNZ_OK )
		{
			[self OutputErrorMessage:@"unzGetCurrentFileInfo"];
			success = NO;
		...
		while( fp )
		{
			...
			{
				[self OutputErrorMessage:@"unzReadCurrentFile"];
				break;
			}
			else 
				break;				
		...
		if( fp )
		{
			...
			if( attr )
			{
				...
				if( ![[NSFileManager defaultManager] setAttributes:attr ofItemAtPath:fullPath error:nil] )
				{
					...
					LogError(@"Failed to set attributes");
				}
			}
		}
		unzCloseCurrentFile( _unzFile );
