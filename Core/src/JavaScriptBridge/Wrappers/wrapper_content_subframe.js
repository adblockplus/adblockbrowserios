...
/**
Equal to wrapper_content_mainframe. Keeping gulp config simple.
May be only one file but gulp mapping is based on filenames (wrapper_* -> main_* -> api_*)
*/

(function() {
  var api = __BROWSERIFIED_API__;
 // window.open in subframe is controversial. It results in some webpages opening new empty tabs
 // because iOS UI has no paradigm for popups. But such misbehavior is less common than pages
 // which are expected to open tabs, like w3schools window opening demo.
 window.open = api.windowOpen;
 /*
  window.close is not allowed in this context.
  DOM spec says that a window is allowed to close itself it if was opened by window.open call.
  A subframe is DOM element which cannot be created by window.open.
 */
}());
